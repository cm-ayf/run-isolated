// https://github.com/InkoHX/sm-discord-bot/blob/main/src/functions/runner/worker/polyfill/setupConsoleLog.js
      ...
      hexSlice = (buffer, start, end) => {
        ...
        try {
          TypedArrayPrototypeForEach(buffer, (v, i) => {
            if (i < start) return;
            s += StringPrototypePadStart(
              NumberPrototypeToString(v, 16),
              2,
              "0"
            );
            if (i >= end) throw null;
...
const kPending = 0,
  kFulfilled = 1,
  kRejected = 2;
export const format = createUtilFormat(
  {
    isAsyncFunction(v) {
